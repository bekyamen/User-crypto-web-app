...
export function CryptoChart({ pair }: CryptoChartProps) {
  ...
  async function fetchHeader() {
    try {
      ...
      if (!res.ok) {
        console.error('Header fetch failed, status:', res.status);
        return;
      }
  ...
  async function loadHistory() {
    try {
      ...
      if (!res.ok) {
        console.error('History fetch failed, status:', res.status);
        return;
      }

      const raw = await res.json();
      if (!Array.isArray(raw)) {
        console.error('Unexpected history response:', raw);
        return;
      }
      ...
      <div className="flex gap-2 mb-4">
        {(['1s', '1m', '15m', '1h', '4h', '1d'] as TF[]).map(x => (
          <button
            key={x}
            onClick={() => setTf(x)}
            className={`px-3 py-1 rounded text-sm ${tf === x ? 'bg-orange-500 text-white' : 'bg-slate-800 text-slate-400'}`}
          >
            {x}
          </button>
        ))}
