...
export default function Header({ notifications = [] }: HeaderProps) {
  ...
  const handleSignOut = () => {
    // If you later add auth cleanup, do it here
    router.push('/login')
  }
  ...
  <div className="w-full px-4 sm:px-6 lg:px-8 py-2"> {/* was py-4 */}
    <div className="flex items-center justify-between">
      ...
      {/* Navigation */}
      <nav className="flex items-center gap-4 sm:gap-6 text-sm flex-1 ml-6">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="p-1.5 hover:bg-slate-800/50 rounded-lg transition text-slate-400 hover:text-white"
        >
          <Settings size={18} />
        </button>
        ...
        <div className="relative" ref={notificationRef}>
          <button
            ...
            className="p-1.5 hover:bg-slate-800/50 rounded-lg transition text-slate-400 hover:text-white relative"
          >
            <Bell size={18} />
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-64 bg-slate-800 border border-slate-700 rounded-lg shadow-lg z-50">
              <div className="p-3 text-white font-semibold border-b border-slate-700">
                Notifications
              </div>
              <ul className="max-h-64 overflow-y-auto">
                {notifications.length > 0 ? (
                  notifications.map((n) => (
                    <li
                      key={n.id}
                      className="px-4 py-2 border-b border-slate-700 hover:bg-slate-700/50 cursor-pointer"
                    >
                      {n.message}
                    </li>
                  ))
        ...
        <button
          ...
          className="p-1.5 hover:bg-slate-800/50 rounded-lg transition text-red-500 hover:text-red-400"
        >
          <LogOut size={18} />
        </button>
      </div>
