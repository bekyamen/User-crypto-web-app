  ...
  useEffect(() => {
    const fetchTickers = async () => {
      try {
        ...
        setTickers(usdtPairs.slice(0, 20))
      } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="relative mb-12 overflow-hidden rounded-2xl border border-slate-800 bg-gradient-to-r from-[#0b1220] to-[#0f1a2e]">
          ...
          <button onClick={prevSlide} className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/40 backdrop-blur-md p-2 rounded-full hover:bg-black/60 transition">
            <ChevronLeft className="w-5 h-5 text-white" />
          </button>
          <button onClick={nextSlide} className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/40 backdrop-blur-md p-2 rounded-full hover:bg-black/60 transition">
            <ChevronRight className="w-5 h-5 text-white" />
          </button>
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
            {carouselSlides.map((_, index) => (
              <button key={index} onClick={() => goToSlide(index)} className={`w-2.5 h-2.5 rounded-full transition ${index === carouselIndex ? 'bg-blue-500 scale-110' : 'bg-slate-600'}`} />
            ))}
          </div>
        </div>
              ...
              <div className="flex items-start justify-between mb-8">
                <div>
                  ...
                  <div className="flex items-baseline gap-2">
                    <h3 className="text-4xl font-bold text-white">{showBalance ? balance?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '••••'}</h3>
